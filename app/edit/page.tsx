  ...
  return (
    <div className='min-h-screen flex flex-col items-center justify-center bg-black text-white px-4'>
      <div className='w-full max-w-md border-2 border-white rounded-2xl p-8 shadow-lg'>
        ...
        <form className='spacy-y-2 flex gap-5 flex-col w-full items-center' onSubmit={hanldeSubmit}>
          <div className='w-[100px] h-[100px] rounded-full border-2 flex justify-center items-center border-white transition-all hover:border-blue-500 text-white hover:text-blue-500 cursor-pointer overflow-hidden relative' onClick={() => imageInput.current?.click()}>
            ...
            {frontendImage ? <Image src={frontendImage || ''} alt='Profiel ' fill /> : <CgProfile size={22} color="white" />}
          </div>
          <div className='w-full '>
            <label className='block mb-1 font-medium'>Name</label>
            <input type='text' placeholder='enter name' className='w-full border-b border-white py-2 px-1 bg-black-900 text-white outline-none placeholder-gray-400' value={name}
